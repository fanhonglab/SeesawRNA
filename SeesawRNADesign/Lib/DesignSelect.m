function seesaw_RNA_switch_seq = DesignSelect(design_list)
    
    dir_list = dir('NUPACKdesigns');
    dir_list(1:2) = [];
    
    not_dir_list = [];
    for n1 = 1:length(dir_list)
        dir_name = ['NUPACKdesigns/',dir_list(n1).name];
        if ~isdir(dir_name)
    ...
    for c = 1:length(dir_list)
        ...
        folder_name = sprintf('NUPACKdesigns/%s',dir_name);
        ...
        for c2 = 1:length(strand_names)
            if issame(strand_names{c2},'hairpin') || issame(strand_names{c2},'rep_hairpin') || issame(strand_names{c2},'switch')||issame(strand_names{c2},'hairpin0')
    ...
    seesaw_RNA_switch_seq = {};
    
    for c1 = 1:length(design_list)
    
        design_name = design_list{c1};
        design_name_rev = design_name(15:end);
    
        design_seq = loadCSVabsFileRawtoStructure(sprintf('NUPACKdesigns/%s.csv',design_name_rev),'');
        design_seq(1,:) = [];
        seesaw_RNA_switch_seq(end+1,:) = design_seq(1,:);
